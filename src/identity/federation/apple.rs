...
mod tests {
    ...
    fn begin_emits_form_post_response_mode() {
        ...
        let state = sample_state("nonce-abc"); // lgtm[rust/hard-coded-cryptographic-value]
    ...
    fn begin_includes_name_email_scope() {
        ...
        let state = sample_state("n"); // lgtm[rust/hard-coded-cryptographic-value]
        let url = conn.begin(&state).expect("begin").0;
        assert!(
    ...
    fn apple_claims_valid() {
        ...
        let state = sample_state("nnn"); // lgtm[rust/hard-coded-cryptographic-value]
        let claims = sample_apple_claims("nnn", 1_700_000_000); // lgtm[rust/hard-coded-cryptographic-value]
    ...
    fn apple_claims_reject_wrong_issuer() {
        let cfg = sample_apple_config_for_claims();
        let state = sample_state("nnn"); // lgtm[rust/hard-coded-cryptographic-value]
        let mut claims = sample_apple_claims("nnn", 1_700_000_000); // lgtm[rust/hard-coded-cryptographic-value]
    ...
    fn apple_claims_reject_nonce_mismatch() {
        let cfg = sample_apple_config_for_claims();
        let state = sample_state("expected"); // lgtm[rust/hard-coded-cryptographic-value]
        let claims = sample_apple_claims("different", 1_700_000_000); // lgtm[rust/hard-coded-cryptographic-value]
        assert!(verify_apple_claims(&claims, &cfg, &state, 1_700_000_000).is_err());
    }

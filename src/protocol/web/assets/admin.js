...
document.addEventListener('alpine:init', () => {
  ...
  // -----------------------------------------------------------------------
  // Login (passkey + conditional mediation)
  // -----------------------------------------------------------------------
  ...
  Alpine.data('passkeyLogin', () => ({
    ...
    passkeyError: null,
    // Server-supplied config: read from data-* attributes on the root.
    beginUrl: '',
    completeUrl: '',
    unavailableMsg: 'Passkey sign-in is not available on this device.',
    cancelledMsg: 'Sign-in cancelled.',
    failedMsg: 'Passkey sign-in failed.',

    init() {
      const ds = this.$el.dataset;
      this.beginUrl = ds.beginUrl || '';
      this.completeUrl = ds.completeUrl || '';
      this.unavailableMsg = ds.unavailableMsg || this.unavailableMsg;
      this.cancelledMsg = ds.cancelledMsg || this.cancelledMsg;
      this.failedMsg = ds.failedMsg || this.failedMsg;
    ...
    async tryConditionalMediation() {
      try {
        ...
        if (assertion) await this.completeAuthentication(assertion);
      } catch (e) {
    ...
    async authenticatePasskey() {
      ...
      try {
        const opts = await this.fetchBeginOptions();
        if (!opts) {
          this.passkeyError = this.unavailableMsg;
          return;
        }
        const assertion = await navigator.credentials.get({ publicKey: opts });
        if (!assertion) {
          this.passkeyError = this.cancelledMsg;
          return;
        }
        await this.completeAuthentication(assertion);
      } catch (e) {
        if (e.name !== 'NotAllowedError') {
          this.passkeyError = e.message || this.failedMsg;
        }
      } finally {
    ...
    async fetchBeginOptions() {
      if (!this.beginUrl) return null;
      const resp = await fetch(this.beginUrl);
    ...
    async completeAuthentication(assertion) {
      ...
      const resp = await fetch(this.completeUrl, {
        method: 'POST',
  ...
  // -----------------------------------------------------------------------
  // Account — passkey registration & rename
  // -----------------------------------------------------------------------
  ...
  Alpine.data('passkeyManager', () => ({
    ...
    async registerPasskey() {
      ...
      try {
        const resp = await fetch('/ui/account/passkeys/register-begin');
        ...
        const credName = prompt('Give this passkey a name (optional):', '') || '';
        const toB64url = (buf) => btoa(String.fromCharCode(...new Uint8Array(buf))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
        const completeResp = await fetch('/ui/account/passkeys/register-complete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  Alpine.data('passkeyRow', (credId) => ({
    ...
    _name: null,
    init() {
      const raw = this.$el.dataset.initialName || '';
      this._name = raw || null;
      this.displayName = raw || credId.substring(0, 16) + '\u2026';
    },
    startEdit() { this.draftName = this._name || ''; this.editing = true; },
    cancelEdit() { this.editing = false; },
    async saveEdit() {
      const name = this.draftName.trim();
      try {
        const resp = await fetch(`/ui/account/passkeys/${this.credId}/rename`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        this.displayName = name || this.credId.substring(0, 16) + '\u2026';
        this.editing = false;
      } catch (e) {

...
- [ ] 25.14 Give the operator an authoring path to a tenant realm's cross-realm trust policies (`POST`/`DELETE /admin/realms/{id}/cross-realm-policies` behind the `scoped_realm` guard); without one the 17.5 deny branch is reachable only through legacy residue, and the operator has no recovery valve (25.11 · MEDIUM)

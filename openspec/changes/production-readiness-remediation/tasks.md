...
- [ ] 4.1 Stop `open()` returning `Ok` after physically destroying every acknowledged record following a mid-segment CRC mismatch (§4.11#3 · P18 · HIGH)
...
- [ ] 5.1 Replace the three hand-written prefix allowlists in `delete_realm` with the key-space sweep the cluster path already uses; `cred:history:` Argon2id hashes and six `audit:*` families survive both branches and are served to the realm ID's next occupant (§4.9#1 · P12 · HIGH)
- [ ] 5.2 Make realm archival a freeze; 11 of 16 mutating engine operations still write an archived realm, including `delete_user`, `set_password` and `register_client` (§4.20#5 · P14 · HIGH)

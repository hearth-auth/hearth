...
- [ ] 25.4 Stop the SDKs sending `PUT` to the five admin mutation routes the server implements as `PATCH`; the PHP admin client 405s at runtime and three more SDKs document the wrong verb (22.6 · HIGH)
- [ ] 25.5 Read and enforce the declared `<ds:CanonicalizationMethod>` and `<ds:Transforms>` on the SAML verify path; `canonicalize(.., true)` is called unconditionally and transforms are never extracted (19.7 · MEDIUM)
- [ ] 25.6 Constrain `<ds:Signature>` discovery to the depth the doc comment claims; `find_element_range` matches the first element at any depth, so `extract_signature_fields` can read a nested signature (19.7 · MEDIUM, possible XSW vector)
- [ ] 25.7 Update `docs/guides/sdks/go.mdx`, `go-echo.md`, `overview.md` and `docs/specs/SDK_SURFACE.md`, which document the pre-25.1 synchronous Go and Rust RBAC signatures (25.1 · LOW)
- [ ] 25.8 Fix the Kotlin `:hearth-ktor:test` sources, which do not compile at HEAD (`Unresolved reference 'call'`), and the 12 pre-existing `:hearth-spring:test` failures (25.1 · MEDIUM, pre-existing)
- [ ] 25.9 Extend the KEK strict-unwrap and enrolment sweep to the SAML keys, DPoP nonce secrets and MFA DEK, which still use the lenient unwrap (18.8 · MEDIUM)
- [ ] 25.10 Remove RS256 and ES256 from the node and TypeScript SDK JWKS verifiers, which accept algorithms Hearth never signs with; leave the Kotlin federation fallback, which is documented and sanctioned by `docs/specs/SDK.md` (18.1 · MEDIUM)
...
- [ ] 25.12 Make `examples/federation-flow` work under the federation SSRF guard; its bundled upstream IdP listens on `http://localhost:9090`, which 22.7 now refuses, so the documented demo login no longer completes (22.7 · MEDIUM)

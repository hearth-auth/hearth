...
| 2 | **Core types** | — | `RealmId`, `UserId`, `SessionId`, `Timestamp`, `Clock` trait, error trait foundations. Everything depends on these. |
...
Phase 1 builds production features on top of the Phase 0 foundation. The dependency chain is: realm isolation → observability → extended authorization → auth methods → transport security → management API → conformance → client SDKs → integration.

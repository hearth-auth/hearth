...
sidebar_label: Rust
description: Verify Hearth tokens and enforce RBAC in a Rust service. Covers Axum, Actix-web Tower middleware, async permission checks, and the auth code + PKCE flow.
---
...
# For Actix-web middleware support, enable the feature:
...
Use `begin_login` / `complete_login` to handle the OAuth callback in two calls. The core SDK calls are identical across frameworks — the tabs show how to wire them into each framework's handler style.
...
async fn login_handler(
    session: Session,
    Extension(client): Extension<HearthClient>,
) -> impl IntoResponse {
    let result = client.begin_login("http://localhost:8080/callback", None).await.unwrap();
...
async fn callback_handler(
    ...
    let code_verifier = session.get::<String>("code_verifier")
        .unwrap()
        .unwrap_or_default();
    let tokens = client.complete_login(
        &params["code"],
        &code_verifier,
...
See the [Actix-web adapter guide](./rust-actix.md) for `HearthActixMiddleware`, `RequirePermission` extractor, and permission delivery modes.
...
round-trip) and `exp`/`nbf`/`iss` are enforced before any claim is read. They
are therefore `async` methods on `&self`, and return `Err` when the token is
malformed, unsigned, expired, not yet valid, or signed by an unknown key:

```rust
let allowed  = client.has_permission(&access_token, "documents.write").await?;
let is_admin = client.has_role(&access_token, "admin").await?;
let in_eng   = client.in_group(&access_token, "engineering").await?;
let in_acme  = client.in_org(&access_token, "org_acme").await?;
```
...
Apply token verification and permission checks using the adapter for your framework. Both adapters respond `401` on a missing or invalid token and `403` on a permission failure.

<Tabs groupId="framework">
<TabItem value="axum" label="Axum (Tower)" default>

Enable the `tower-middleware` feature, then apply `RequirePermissionLayer` to an Axum router:
...
`RequirePermissionLayer` works with any Tower-compatible framework (Axum, Tonic, or any service using the `tower` crate).
...
Enable the `actix-middleware` feature, then use `HearthActixMiddleware` and `RequirePermission` on your scopes:
    ...
    App::new()
        ...
        .service(
            web::scope("/admin")
                ...
                .wrap(RequirePermission::new("admin.write"))
                .route("/users", web::get().to(list_users))
        )
        .service(
            web::scope("/docs")
                .wrap(HearthActixMiddleware::new(client.clone(), AccessTokenAuthorization::Embedded))
                .wrap(RequirePermission::new("docs.write"))
...
See the [Actix-web guide](./rust-actix.md) for `RequirePermission` as an extractor, scope protection, and introspection mode configuration.
...
| `has_permission(token, permission).await` | Verifies, then reads the `permissions` claim |
| `has_role(token, role).await` | Verifies, then reads the `roles` claim |
| `in_group(token, group_slug).await` | Verifies, then reads the `groups` claim |
| `in_org(token, org_id).await` | Verifies, then reads the `oid` claim |

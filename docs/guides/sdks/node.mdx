...
description: Verify Hearth tokens and enforce RBAC in a Node.js server in under 5 minutes. Covers Express, Fastify, and Next.js middleware, JWKS caching, and the Admin API.
---
...
This is the **server-side** SDK. Use it to handle the OAuth callback route, verify incoming Bearer tokens, protect Express/Fastify/Next.js routes, and call the Admin API.
...
For Node.js servers that handle the OAuth callback, use `beginLogin` / `completeLogin`. The core SDK calls are the same across frameworks — the tabs show how to declare the routes in each.
...
app.get("/callback", async (request, reply) => {
  ...
  if (state !== request.session.oauthState) {
    return reply.status(400).send("state mismatch");
  }
  const tokens = await client.completeLogin(
    code,
...
export async function GET() {
  ...
  cookieStore.set("oauth_state",    state,        { httpOnly: true, sameSite: "lax" });
  cookieStore.set("code_verifier",  codeVerifier, { httpOnly: true, sameSite: "lax" });
...
See the [Next.js adapter guide](./node-nextjs.md) for Edge Runtime middleware, Pages Router patterns, and `getHearthToken` for Server Components.
...
Apply token verification and permission checks to your routes. All three adapters respond `401` on a missing or invalid token and `403` on a permission failure.
...
`hearthFastifyHook` integrates with Fastify's `preHandler` lifecycle. Non-HTTP connections (WebSocket) pass through without token checks.
...
See the [Next.js adapter guide](./node-nextjs.md) for App Router Server Components, Pages Router `withHearthAuth`, and per-route permission guards.

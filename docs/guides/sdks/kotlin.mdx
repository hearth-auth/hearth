...
@Configuration
class HearthSecurityConfig(
    private val hearthFilter: HearthJwtAuthenticationFilter,
    private val hearthEntryPoint: AuthenticationEntryPoint,   // auto-configured
) {
    @Bean
    fun securityFilterChain(http: HttpSecurity): SecurityFilterChain {
        http
            ...
            // Required: a chain with no httpBasic/formLogin/oauth2ResourceServer gets
            // Spring Security's Http403ForbiddenEntryPoint, so a request with no token
            // is answered 403 instead of 401.
...
class SecurityConfig(
    ...
    fun securityFilterChain(http: HttpSecurity) = http
        ...
        .addFilterBefore(hearthFilter, UsernamePasswordAuthenticationFilter::class.java)
        // Required, or a tokenless request is answered 403 rather than 401.
        .exceptionHandling { it.authenticationEntryPoint(hearthEntryPoint) }
        .build()!!
}

...
let admin = client.admin(admin_token);

admin.assign_role(hearth_sdk::AssignRoleRequest {
    user_id: "<user-id>".to_string(),
    role_id: "<role-id>".to_string(),
    scope:   Some(hearth_sdk::OrgScope { org_id: "org_<uuid>".to_string() }),
}).await?;
```
...
admin.assignRole(
    userId = "<user-id>",
    roleId = "<role-id>",
    scope  = OrgScope(orgId = "org_<uuid>"),
)
```
...
```php
$perms = $hearth->mePermissions($accessToken, orgId: 'org_<uuid>');
// $perms->roles, $perms->groups, $perms->permissions — includes org-scoped entries
```
...
```rust
let perms = client.me_permissions(&access_token, Some("org_<uuid>")).await?;
// perms.roles, perms.groups, perms.permissions — includes org-scoped entries
```
...
```kotlin
val perms = client.mePermissions(accessToken, orgId = "org_<uuid>")
// perms.roles, perms.groups, perms.permissions — includes org-scoped entries
```

...
Hearth has two independent webhook mechanisms:

1. **Audit event webhooks** (this guide) — subscribe to real-time events delivered by the
   audit engine. Configured at runtime via `POST /admin/webhooks`.
2. **Pre-token enrichment webhook** — called before every access token issuance to inject
   custom claims. Configured statically in `hearth.yaml` under
   `realms.<name>.pre_token_webhook`. See
   [CONFIGURATION.md §realms.\<name\>.pre_token_webhook](../specs/CONFIGURATION.md) for
   the full reference.
:::
